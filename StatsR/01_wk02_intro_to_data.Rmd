...
print("No flight is delayed more than 2 hours.")
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(mean_ad = mean(dep_delay), iqr_ad = IQR(arr_delay), n = n())
...
nycflights <- nycflights %>% mutate(avg_speed=(air_time*60)/distance)

nycflights %>% arrange(avg_speed) %>% select(tailnum)
...
print("As distance increases the average speed of flights decreases.")
```
...
# For dep delayed flights:

del_dep_ot_arr_rate <- nycflights %>%  
  filter(dep_type == "delayed") %>%
  summarise(del_dep_ot_arr_rate = sum(arr_type == "on time")/n())

paste("What proportion of flights that departed late arrive on time? ", del_dep_ot_arr_rate, sep=" ")

# For all flights:

flights_summary <- nycflights %>%
  summarise(count_flights = n())

ot_arr_count <- nycflights %>%  
summarise(ot_arr_count = sum(arr_type == "on time"))

flights_summary <- merge(flights_summary, ot_arr_count)

ot_arr_del_dep_count <- nycflights %>%  
  filter(dep_type == "delayed") %>%
  summarise(ot_arr_del_dep_count = sum(arr_type == "on time"))

flights_summary <- merge(flights_summary, ot_arr_del_dep_count)

flights_summary <- flights_summary %>% 
  mutate(ot_arr_rate = ot_arr_count/count_flights)

paste("What proportion of all flights arrive on time? ", flights_summary$ot_arr_rate, sep=" ")

flights_summary <- flights_summary %>% 
  mutate(ot_arr_dep_del_rate = ot_arr_del_dep_count/count_flights)

paste("What proportion of all flights depart late and arrive on time? ", flights_summary$ot_arr_dep_del_rate, sep=" ")
