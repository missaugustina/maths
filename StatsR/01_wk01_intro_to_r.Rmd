...
more_girls_count <- count(subset(present, more_boys == FALSE))
more_boys_count <- count(subset(present, more_boys == TRUE))

# Every year there are more girls born than boys.
if(more_girls_count > more_boys_count){
  print("Every year there are more girls born than boys.")
} else if( more_boys_count > more_girls_count) {
...
ggplot(present, aes(year, y = prop_boy_girl)) +
  geom_line() +
  geom_point()

print("There is initially a decrease in the boy-to-girl ratio, and then an increase between 1960 and 1970, followed by a decrease.")
...
head(arrange(present, desc(total)),1)$year
```
